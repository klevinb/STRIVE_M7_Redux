...
class App extends Component {
  ...
  fetchData = async (limit, offset) => {
    const response = await fetch(
      "http://127.0.0.1:3003/students?limit=" + limit + "&offset=" + offset
    );
  ...
  fetchUser = async (id, name, surname) => {
    const resp = await fetch(
      "http://127.0.0.1:3003/students/" + id + "/getPhoto"
    );
    if (resp.ok) {
  ...
  addNewStudent = async (e) => {
    ...
    const resp2 = await fetch(
      "http://127.0.0.1:3003/students/" + studentId._id + "/uploadPhoto",
      {
        method: "POST",
        body: data,
      }
  ...
  editStudent = async (e) => {
    ...
    const resp = await fetch(
      "http://127.0.0.1:3003/students/" + this.state.newStudent._id,
      {
        method: "PUT",
        body: JSON.stringify(this.state.newStudent),
        headers: {
